...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    updateSession: (state, action: PayloadAction<{ sessionId: number, updates: Partial<Session> }>) => {
      const index = state.sessions.findIndex(session => session.id === action.payload.sessionId)
      if (index !== -1) {
    ...
    addMessage: (state, action: PayloadAction<{ sessionId: number, message: Message }>) => {
